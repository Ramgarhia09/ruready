...
import { Menu, Home, Grid, MessageCircle, User, Camera, Edit2, Save, X, Trash2 } from 'lucide-react';
import { 
  getFirestore, 
  doc, 
  getDoc, 
...
const ProfilePage = () => {
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      // Try to delete from storage only if it's a custom uploaded image
      // (Skip if it's a default Google/Facebook photo or ui-avatars)
      if (profileData.photoURL.includes('firebase') || profileData.photoURL.includes(currentUser.uid)) {
        const photoRef = ref(storage, profileData.photoURL);
        await deleteObject(photoRef).catch((err) => {
          console.warn('Storage delete failed (possibly already deleted or not in storage):', err);
        });
      }

      // Clear photoURL from Auth and Firestore
    ...
    <div className="min-h-screen bg-pink-50 flex">
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-white shadow-lg z-40">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => navigate('/dashboard')} className="w-full flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-pink-50 transition-colors text-left group">
          ...
          <button onClick={() => navigate('/reels')} className="w-full flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-pink-50 transition-colors text-left group">
          ...
          <button onClick={() => navigate('/userList')} className="w-full flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-pink-50 transition-colors text-left group">
          ...
          <button onClick={() => navigate('/myprofile')} className="w-full flex items-center gap-4 px-4 py-3 rounded-lg bg-pink-50 transition-colors text-left">
        ...
        <div className="p-4 m-4 bg-gradient-to-br from-pink-500 to-purple-500 rounded-xl text-white">
          ...
          <button onClick={() => navigate('/gopremium')} className="w-full bg-white text-pink-500 py-2 px-4 rounded-lg font-semibold text-sm hover:bg-gray-100 transition-colors">
            Upgrade
          </button>
        </div>
      </aside>
            ...
            <div className="bg-white rounded-3xl shadow-lg overflow-hidden">
              ...
              <div className="bg-gradient-to-b from-pink-100 to-white p-8 text-center">
                <div className="relative inline-block">
                  <div className="w-32 h-32 sm:w-40 sm:h-40 bg-purple-200 rounded-full flex items-center justify-center overflow-hidden shadow-xl border-4 border-white">
                  ...
                  {/* Upload Button */}
                  {isEditing && (
                    <label className={`absolute bottom-0 right-0 bg-pink-500 hover:bg-pink-600 p-3 rounded-full cursor-pointer transition-colors shadow-lg ${uploadingImage || deletingImage ? 'opacity-50 cursor-not-allowed' : ''}`}>
                      {uploadingImage ? (
                  ...
                  {/* Delete Button - Only when photo exists and editing */}
                  {isEditing && profileData.photoURL && (
                    <button
                      onClick={() => setShowDeleteConfirm(true)}
                      disabled={uploadingImage || deletingImage}
                      className="absolute bottom-0 left-0 bg-red-500 hover:bg-red-600 p-3 rounded-full transition-colors shadow-lg"
                      title="Delete photo"
                    >
                      {deletingImage ? (
                        <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                ...
                {(uploadingImage || deletingImage) && (
                  <p className="mt-4 text-sm text-gray-600">
                    {uploadingImage ? 'Uploading...' : 'Removing photo...'}
                  </p>
                )}
              </div>
              ...
              <div className="p-6 sm:p-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8">
                  ...
                  <ProfileField label="Email" value={profileData.email} isEditing={false} />
                  ...
                  <ProfileField label="Gender" value={profileData.gender} isEditing={isEditing} onChange={(v) => updateField('gender', v)} type="select" />
                  <ProfileField label="Preference" value={profileData.preference} isEditing={isEditing} onChange={(v) => updateField('preference', v)} type="select" />
                ...
                <div className="py-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {profileData.plan === 'Free' && (
                      <button onClick={() => navigate('/gopremium')} className="text-pink-500 hover:text-pink-600 font-semibold text-sm">
                        Upgrade →
                      </button>
                    )}
                  </div>
      ...
      <nav className="lg:hidden bg-pink-500 shadow-lg fixed bottom-0 left-0 right-0 z-40">
        <div className="flex justify-around items-center py-3">
          <button onClick={() => navigate('/dashboard')} className="flex flex-col items-center gap-1 text-white/60">
            <Home size={24} /><span className="text-xs font-medium">Home</span>
          </button>
          <button onClick={() => navigate('/reels')} className="flex flex-col items-center gap-1 text-white/60">
            <Grid size={24} /><span className="text-xs font-medium">Reels</span>
          </button>
          <button onClick={() => navigate('/userList')} className="flex flex-col items-center gap-1 text-white/60">
            <MessageCircle size={24} /><span className="text-xs font-medium">Chat</span>
          </button>
      ...
      {showMenu && (
        <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setShowMenu(false)}>
          <div className="absolute right-0 top-0 bottom-0 w-80 bg-white shadow-2xl overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4">
              <button onClick={() => { setShowMenu(false); navigate('/gopremium'); }} className="w-full flex items-center gap-4 px-4 py-4 hover:bg-gray-50 rounded-lg transition-colors text-left">
                <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
                  <svg className="w-6 h-6 text-purple-500" fill="currentColor" viewBox="0 0 20 20"><path d="M5 3a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2H5zM5 11a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2H5zM11 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zM14 11a1 1 0 011 1v1h1a1 1 0 110 2h-1v1a1 1 0 11-2 0v-1h-1a1 1 0 110-2h1v-1a1 1 0 011-1z" /></svg>
                </div>
                <span className="text-gray-700 font-medium text-lg">Go Premium</span>
              </button>

              <button onClick={() => { setShowMenu(false); navigate('/setting'); }} className="w-full flex items-center gap-4 px-4 py-4 hover:bg-gray-50 rounded-lg transition-colors text-left">
                <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
                  <svg className="w-6 h-6 text-gray-600" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd" /></svg>
                </div>
                <span className="text-gray-700 font-medium text-lg">Settings</span>
              </button>

              <button onClick={() => { setShowMenu(false); navigate('/privacysafety'); }} className="w-full flex items-center gap-4 px-4 py-4 hover:bg-gray-50 rounded-lg transition-colors text-left">
              ...
              <button onClick={() => { setShowMenu(false); navigate('/helpcenter'); }} className="w-full flex items-center gap-4 px-4 py-4 hover:bg-gray-50 rounded-lg transition-colors text-left">
                <div className="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center">
                  <svg className="w-6 h-6 text-orange-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" /></svg>
                </div>
              ...
              <button onClick={handleLogout} className="w-full flex items-center gap-4 px-4 py-4 hover:bg-red-50 rounded-lg transition-colors text-left">
                <div className="w-10 h-10 bg-red-100 rounded-lg flex items-center justify-center">
                  <svg className="w-6 h-6 text-red-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3 3a1 1 0 00-1 1v12a1 1 0 102 0V4a1 1 0 00-1-1zm10.293 9.293a1 1 0 001.414 1.414l3-3a1 1 0 000-1.414l-3-3a1 1 0 10-1.414 1.414L14.586 9H7a1 1 0 100 2h7.586l-1.293 1.293z" clipRule="evenodd" /></svg>
                </div>
